...
class SaveAnyMonitor:
    ...
    def create_settings_tab(self, parent):
        ...
        # 全局配置框架
        global_frame = ttk.LabelFrame(parent, text="全局配置", padding="10")
        global_frame.pack(fill=tk.X, pady=(0, 10))
        
        # Stream 模式
        stream_row = ttk.Frame(global_frame)
        stream_row.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(stream_row, text="Stream 模式:").pack(side=tk.LEFT)
        self.stream_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(stream_row, text="启用 Stream 模式", variable=self.stream_var).pack(side=tk.LEFT, padx=(5, 0))
        
        # 下载线程数
        threads_row = ttk.Frame(global_frame)
        threads_row.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(threads_row, text="下载线程数:", width=10).pack(side=tk.LEFT)
        self.threads_entry = ttk.Entry(threads_row, width=10)
        self.threads_entry.insert(0, "4")
        self.threads_entry.pack(side=tk.LEFT, padx=(5, 0))
        ttk.Label(threads_row, text="(默认: 4)", foreground="gray").pack(side=tk.LEFT, padx=(5, 0))
        
        # 重试次数
        retry_row = ttk.Frame(global_frame)
        retry_row.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(retry_row, text="重试次数:", width=10).pack(side=tk.LEFT)
        self.retry_entry = ttk.Entry(retry_row, width=10)
        self.retry_entry.insert(0, "3")
        self.retry_entry.pack(side=tk.LEFT, padx=(5, 0))
        ttk.Label(retry_row, text="(默认: 3)", foreground="gray").pack(side=tk.LEFT, padx=(5, 0))
        
        # 全局代理
        global_proxy_row = ttk.Frame(global_frame)
        global_proxy_row.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(global_proxy_row, text="全局代理:", width=10).pack(side=tk.LEFT)
        self.global_proxy_entry = ttk.Entry(global_proxy_row, width=50)
        self.global_proxy_entry.pack(side=tk.LEFT, padx=(5, 0), fill=tk.X, expand=True)
        ttk.Label(global_proxy_row, text="(可选)", foreground="gray").pack(side=tk.LEFT, padx=(5, 0))
        
        # 全局配置按钮
        global_btn_row = ttk.Frame(global_frame)
        global_btn_row.pack(fill=tk.X, pady=(5, 0))
        ttk.Button(global_btn_row, text="从配置加载", command=self.load_global_from_config).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(global_btn_row, text="保存到配置", command=self.save_global_to_config).pack(side=tk.LEFT)
        
        # Telegram 高级配置框架
        telegram_frame = ttk.LabelFrame(parent, text="Telegram 高级配置", padding="10")
        telegram_frame.pack(fill=tk.X, pady=(0, 10))
        
        # App ID
        app_id_row = ttk.Frame(telegram_frame)
        app_id_row.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(app_id_row, text="App ID:", width=10).pack(side=tk.LEFT)
        self.app_id_entry = ttk.Entry(app_id_row, width=20)
        self.app_id_entry.pack(side=tk.LEFT, padx=(5, 0))
        
        # App Hash
        app_hash_row = ttk.Frame(telegram_frame)
        app_hash_row.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(app_hash_row, text="App Hash:", width=10).pack(side=tk.LEFT)
        self.app_hash_entry = ttk.Entry(app_hash_row, width=50)
        self.app_hash_entry.pack(side=tk.LEFT, padx=(5, 0), fill=tk.X, expand=True)
        
        # Flood Retry
        flood_retry_row = ttk.Frame(telegram_frame)
        flood_retry_row.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(flood_retry_row, text="Flood 重试:", width=10).pack(side=tk.LEFT)
        self.flood_retry_entry = ttk.Entry(flood_retry_row, width=10)
        self.flood_retry_entry.insert(0, "5")
        self.flood_retry_entry.pack(side=tk.LEFT, padx=(5, 0))
        ttk.Label(flood_retry_row, text="(默认: 5)", foreground="gray").pack(side=tk.LEFT, padx=(5, 0))
        
        # RPC Retry
        rpc_retry_row = ttk.Frame(telegram_frame)
        rpc_retry_row.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(rpc_retry_row, text="RPC 重试:", width=10).pack(side=tk.LEFT)
        self.rpc_retry_entry = ttk.Entry(rpc_retry_row, width=10)
        self.rpc_retry_entry.insert(0, "5")
        self.rpc_retry_entry.pack(side=tk.LEFT, padx=(5, 0))
        ttk.Label(rpc_retry_row, text="(默认: 5)", foreground="gray").pack(side=tk.LEFT, padx=(5, 0))
        
        # Telegram 配置按钮
        telegram_btn_row = ttk.Frame(telegram_frame)
        telegram_btn_row.pack(fill=tk.X, pady=(5, 0))
        ttk.Button(telegram_btn_row, text="从配置加载", command=self.load_telegram_from_config).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(telegram_btn_row, text="保存到配置", command=self.save_telegram_to_config).pack(side=tk.LEFT)
        
        # 杂项配置框架
        misc_frame = ttk.LabelFrame(parent, text="杂项配置", padding="10")
        misc_frame.pack(fill=tk.X, pady=(0, 10))
        
        # 不清空缓存
        no_clean_cache_row = ttk.Frame(misc_frame)
        no_clean_cache_row.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(no_clean_cache_row, text="缓存管理:").pack(side=tk.LEFT)
        self.no_clean_cache_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(no_clean_cache_row, text="退出时不清空缓存", variable=self.no_clean_cache_var).pack(side=tk.LEFT, padx=(5, 0))
        
        # 临时文件夹
        temp_path_row = ttk.Frame(misc_frame)
        temp_path_row.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(temp_path_row, text="临时文件夹:", width=10).pack(side=tk.LEFT)
        self.temp_path_entry = ttk.Entry(temp_path_row, width=50)
        self.temp_path_entry.insert(0, "./cache")
        self.temp_path_entry.pack(side=tk.LEFT, padx=(5, 0), fill=tk.X, expand=True)
        ttk.Button(temp_path_row, text="浏览...", command=self.browse_temp_path).pack(side=tk.LEFT, padx=(5, 0))
        
        # 杂项配置按钮
        misc_btn_row = ttk.Frame(misc_frame)
        misc_btn_row.pack(fill=tk.X, pady=(5, 0))
        ttk.Button(misc_btn_row, text="从配置加载", command=self.load_misc_from_config).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(misc_btn_row, text="保存到配置", command=self.save_misc_to_config).pack(side=tk.LEFT)
...
cache_path = "{cache_path}"\n'''
    ...
    def browse_temp_path(self):
        """浏览临时文件夹"""
        path = filedialog.askdirectory(title="选择临时文件夹")
        if path:
            self.temp_path_entry.delete(0, tk.END)
            self.temp_path_entry.insert(0, path)
    
    def save_global_to_config(self):
        """保存全局配置到配置文件"""
        global config_path
        if not config_path or not os.path.exists(config_path):
            messagebox.showwarning("警告", "请先选择 SaveAny-Bot 程序路径")
        ...
        try:
            ...
            stream = 'true' if self.stream_var.get() else 'false'
            threads = self.threads_entry.get().strip() or "4"
            retry = self.retry_entry.get().strip() or "3"
            proxy = self.global_proxy_entry.get().strip()
            
            # 更新或添加全局配置
            if re.search(r'^stream\s*=', content, re.MULTILINE):
                content = re.sub(r'^(stream\s*=\s*)(true|false)', f'\1{stream}', content, flags=re.MULTILINE)
            else:
                content = f'stream = {stream}\n' + content
            
            if re.search(r'^threads\s*=', content, re.MULTILINE):
                content = re.sub(r'^(threads\s*=\s*)\d+', f'\1{threads}', content, flags=re.MULTILINE)
            else:
                content = re.sub(r'^(stream\s*=)', f'threads = {threads}\n\1', content, flags=re.MULTILINE)
            
            if re.search(r'^retry\s*=', content, re.MULTILINE):
                content = re.sub(r'^(retry\s*=\s*)\d+', f'\1{retry}', content, flags=re.MULTILINE)
            else:
                content = re.sub(r'^(threads\s*=)', f'retry = {retry}\n\1', content, flags=re.MULTILINE)
            
            if proxy:
                if re.search(r'^proxy\s*=', content, re.MULTILINE):
                    content = re.sub(r'^(proxy\s*=\s*)["\'][^"\']["\']', f'\\1\"{proxy}\"', content, flags=re.MULTILINE)
                else:
                    content = re.sub(r'^(retry\s*=)', f'proxy = "{proxy}"\n\1', content, flags=re.MULTILINE)
    ...
    def load_global_from_config(self):
        """从配置文件加载全局配置"""
        global config_path
        if not config_path or not os.path.exists(config_path):
            messagebox.showwarning("警告", "请先选择 SaveAny-Bot 程序路径")
        ...
        try:
            ...
            stream_match = re.search(r'^stream\s*=\s*(true|false)', content, re.MULTILINE | re.IGNORECASE)
            if stream_match:
                self.stream_var.set(stream_match.group(1).lower() == 'true')
            
            threads_match = re.search(r'^threads\s*=\s*(\d+)', content, re.MULTILINE)
            if threads_match:
                self.threads_entry.delete(0, tk.END)
                self.threads_entry.insert(0, threads_match.group(1))
            
            retry_match = re.search(r'^retry\s*=\s*(\d+)', content, re.MULTILINE)
            if retry_match:
            ...
            messagebox.showinfo("成功", "全局配置已加载！")
            self.log("已加载全局配置")
        except Exception as e:
            messagebox.showerror("错误", f"加载失败: {str(e)}")
    
    def save_telegram_to_config(self):
        """保存 Telegram 配置到配置文件"""
        global config_path
        if not config_path or not os.path.exists(config_path):
            messagebox.showwarning("警告", "请先选择 SaveAny-Bot 程序路径")
        ...
        try:
            ...
            app_hash = self.app_hash_entry.get().strip()
            flood_retry = self.flood_retry_entry.get().strip() or "5"
            rpc_retry = self.rpc_retry_entry.get().strip() or "5"
            
            # 查找 [telegram] 部分
            telegram_section_match = re.search(r'(\[telegram\].*?)(?=\[|$)', content, re.DOTALL)
            if telegram_section_match:
                telegram_section = telegram_section_match.group(1)
                
                if app_id:
                    if re.search(r'app_id\s*=', telegram_section):
                        telegram_section = re.sub(r'(app_id\s*=\s*)\d+', f'\1{app_id}', telegram_section)
                    else:
                        telegram_section += f'app_id = {app_id}\n'
                ...
                if re.search(r'flood_retry\s*=', telegram_section):
                    telegram_section = re.sub(r'(flood_retry\s*=\s*)\d+', f'\1{flood_retry}', telegram_section)
                else:
                    telegram_section += f'flood_retry = {flood_retry}\n'
                
                if re.search(r'rpc_retry\s*=', telegram_section):
                    telegram_section = re.sub(r'(rpc_retry\s*=\s*)\d+', f'\1{rpc_retry}', telegram_section)
                else:
                    telegram_section += f'rpc_retry = {rpc_retry}\n'
                
                content = content.replace(telegram_section_match.group(0), telegram_section)
            
            with open(config_path, 'w', encoding='utf-8') as f:
                f.write(content)
            
            messagebox.showinfo("成功", "Telegram 配置已保存！")
            self.log("已保存 Telegram 配置")
        except Exception as e:
            messagebox.showerror("错误", f"保存失败: {str(e)}")
    
    def load_telegram_from_config(self):
        """从配置文件加载 Telegram 配置"""
        global config_path
        if not config_path or not os.path.exists(config_path):
            messagebox.showwarning("警告", "请先选择 SaveAny-Bot 程序路径")
        ...
        try:
            ...
            app_id_match = re.search(r'\[telegram\].*?app_id\s*=\s*(\d+)', content, re.DOTALL)
            if app_id_match:
            ...
            flood_retry_match = re.search(r'\[telegram\].*?flood_retry\s*=\s*(\d+)', content, re.DOTALL)
            if flood_retry_match:
                self.flood_retry_entry.delete(0, tk.END)
                self.flood_retry_entry.insert(0, flood_retry_match.group(1))
            
            rpc_retry_match = re.search(r'\[telegram\].*?rpc_retry\s*=\s*(\d+)', content, re.DOTALL)
            if rpc_retry_match:
                self.rpc_retry_entry.delete(0, tk.END)
                self.rpc_retry_entry.insert(0, rpc_retry_match.group(1))
            
            messagebox.showinfo("成功", "Telegram 配置已加载！")
            self.log("已加载 Telegram 配置")
        except Exception as e:
            messagebox.showerror("错误", f"加载失败: {str(e)}")
    
    def save_misc_to_config(self):
        """保存杂项配置到配置文件"""
        global config_path
        if not config_path or not os.path.exists(config_path):
            messagebox.showwarning("警告", "请先选择 SaveAny-Bot 程序路径")
        ...
        try:
            ...
            no_clean_cache = 'true' if self.no_clean_cache_var.get() else 'false'
            temp_path = self.temp_path_entry.get().strip()
            
            if re.search(r'^no_clean_cache\s*=', content, re.MULTILINE):
                content = re.sub(r'^(no_clean_cache\s*=\s*)(true|false)', f'\1{no_clean_cache}', content, flags=re.MULTILINE | re.IGNORECASE)
            else:
                content += f'\nno_clean_cache = {no_clean_cache}\n'
            
            if temp_path:
                if re.search(r'\[temp\]', content):
                    temp_section_match = re.search(r'(\[temp\].*?)(?=\[|$)', content, re.DOTALL)
            ...
            messagebox.showinfo("成功", "杂项配置已保存！")
            self.log("已保存杂项配置")
        except Exception as e:
            messagebox.showerror("错误", f"保存失败: {str(e)}")
    
    def load_misc_from_config(self):
        """从配置文件加载杂项配置"""
        global config_path
        if not config_path or not os.path.exists(config_path):
            messagebox.showwarning("警告", "请先选择 SaveAny-Bot 程序路径")
        ...
        try:
            ...
            no_clean_cache_match = re.search(r'^no_clean_cache\s*=\s*(true|false)', content, re.MULTILINE | re.IGNORECASE)
            ...
            messagebox.showinfo("成功", "杂项配置已加载！")
            self.log("已加载杂项配置")
        except Exception as e:
            messagebox.showerror("错误", f"加载失败: {str(e)}")
