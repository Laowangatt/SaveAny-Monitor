...
class SaveAnyMonitor:
    ...
    def create_monitor_tab(self, parent):
        ...
        path_text = self.target_path if self.target_path else "自动检测"
        self.path_label = ttk.Label(path_row, text=path_text, wraplength=500)
    ...
    def create_log_tab(self, parent):
        ...
        ttk.Checkbutton(btn_frame, text="自动滚动", variable=self.auto_scroll_var).pack(side=tk.LEFT, padx=(0, 20))
        
        ttk.Button(btn_frame, text="清空显示", command=self.clear_console_log).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(btn_frame, text="打开日志文件夹", command=self.open_log_folder).pack(side=tk.LEFT)
        
        path_frame = ttk.Frame(parent)
        path_frame.pack(fill=tk.X, pady=(0, 10))
        ttk.Label(path_frame, text="当前日志文件:").pack(side=tk.LEFT)
        self.log_path_label = ttk.Label(path_frame, text="未开始记录", foreground="gray")
        self.log_path_label.pack(side=tk.LEFT, padx=(5, 0))
        
        # 日志文本框
        self.console_log = scrolledtext.ScrolledText(parent, wrap=tk.WORD, font=("Consolas", 9), background="#000", foreground="#0f0")
    ...
    def create_settings_tab(self, parent):
        ...
        ttk.Label(proxy_url_row, text="代理 URL:", width=12).pack(side=tk.LEFT)
        self.proxy_url_entry = ttk.Entry(proxy_url_row)
        self.proxy_url_entry.pack(side=tk.LEFT, fill=tk.X, expand=True)
        
        # 测试按钮
        proxy_test_row = ttk.Frame(proxy_frame)
        proxy_test_row.pack(fill=tk.X)
        ttk.Button(proxy_test_row, text="测试连接", command=self.test_proxy_connection).pack(side=tk.LEFT)
        self.proxy_status_label = ttk.Label(proxy_test_row, text="")
        self.proxy_status_label.pack(side=tk.LEFT, padx=(10, 0))
        
        # 存储设置
        storage_frame = ttk.LabelFrame(parent, text="存储路径设置 [storage]", padding="10")
        storage_frame.pack(fill=tk.X, pady=(0, 10))
        
        # 本地路径
        local_path_row = ttk.Frame(storage_frame)
        local_path_row.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(local_path_row, text="本地路径:", width=12).pack(side=tk.LEFT)
        self.local_path_entry = ttk.Entry(local_path_row)
        ...
        ttk.Button(action_frame, text="从配置加载", command=self.load_settings_from_config).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(action_frame, text="保存到配置", command=self.save_settings_to_config).pack(side=tk.LEFT)
    ...
    def auto_detect_exe_path(self):
        """自动检测saveany-bot.exe的路径"""
        import os
        search_paths = []
        ...
        # 常见的安装目录
        common_paths = [
            os.path.join(current_dir, 'SaveAny-Bot'),
            os.path.join(os.path.expanduser('~'), 'Desktop'),
            os.path.join(os.path.expanduser('~'), 'Downloads'),
            os.path.join(os.environ.get('ProgramFiles', r'C:\Program Files'), 'SaveAny-Bot'),
            os.path.join(os.environ.get('ProgramFiles(x86)', r'C:\Program Files (x86)'), 'SaveAny-Bot')
        ]
        ...
        for search_path in search_paths:
            if os.path.exists(search_path):
                for root, dirs, files in os.walk(search_path):
                    for file in files:
                        if file.lower() == self.target_process.lower():
                            self.target_path = os.path.join(root, file)
                            self.log(f"自动检测到SaveAny-Bot路径: {self.target_path}")
                            return
        ...
        if proc:
            try:
                if hasattr(proc, 'exe'):
                    ...
                    self.log(f"从运行进程获取SaveAny-Bot路径: {self.target_path}")
                elif proc.info.get('exe'):
                    self.target_path = proc.info['exe']
                    self.log(f"从运行进程获取SaveAny-Bot路径: {self.target_path}")
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                pass
        
        if not self.target_path:
            self.log("未检测到SaveAny-Bot路径，请手动选择")
    ...
    def save_config(self):
        global config_path
        if not config_path:
            messagebox.showwarning("警告", "配置文件路径未设置")
            return
    ...
    def test_proxy_connection(self):
        ...
        def do_test():
            try:
                ...
                sock.close()
                self.root.after(0, lambda: self.proxy_status_label.config(text="连接成功 (服务器可达)", foreground="green"))
            except Exception as e:
                self.root.after(0, lambda: self.proxy_status_label.config(text=f"连接失败: {str(e)}", foreground="red"))
    ...
    def load_settings_from_config(self):
        # 简单逻辑：从 config_editor 文本中正则提取
        content = self.config_editor.get('1.0', tk.END)
        
        # 提取代理
        proxy_match = re.search(r'\[telegram\.proxy\].*?enable\s*=\s*(true|false)', content, re.S)
        if proxy_match:
            self.proxy_enable_var.set(proxy_match.group(1) == 'true')
        
        url_match = re.search(r'\[telegram\.proxy\].*?url\s*=\s*"(.*?)"', content, re.S)
        if url_match:
            self.proxy_url_entry.delete(0, tk.END)
            self.proxy_url_entry.insert(0, url_match.group(1))
            
        # 提取路径
        local_match = re.search(r'\[storage\].*?local_path\s*=\s*"(.*?)"', content, re.S)
        if local_path_match := local_match:
            self.local_path_entry.delete(0, tk.END)
            self.local_path_entry.insert(0, local_path_match.group(1))
